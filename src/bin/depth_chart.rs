...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let usdc_faucet_id = env::var("USDC_FAUCET_ID")
        .expect("USDC_FAUCET_ID must be set in .env file");
    let eth_faucet_id = env::var("ETH_FAUCET_ID")
        .expect("ETH_FAUCET_ID must be set in .env file");
    ...
    println!("║                                             OPEN ORDERS                                                  ║");
    println!("╠════════════════════════════════════╦══════════════════╦══════════════════╦═══════════╦═══════════════════╣");
    println!("║             Note ID                ║   Offered Asset  ║ Requested Asset  ║   Type    ║     Creator       ║");
    println!("╠════════════════════════════════════╬══════════════════╬══════════════════╬═══════════╬═══════════════════╣");
    ...
    for order in &open_orders {
        ...
        let offered_asset = note.assets().iter().next().expect("note has no assets").unwrap_fungible();
        ...
        let requested_asset_id = AccountId::try_from([requested_asset_word[3], requested_asset_word[2]])?;
        ...
        println!(
            ...
            format!("{} {}", offered_asset.amount(),
                if offered_asset.faucet_id() == usdc_faucet { "USDC" } else { "ETH" }),
            format!("{} {}", requested_amount,
                if requested_asset_id == usdc_faucet { "USDC" } else { "ETH" }),
            order_type,
            trader_name
        );
